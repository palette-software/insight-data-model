		...
		v_sql := v_sql || '
			with t_requests as 
                ...
                where 1 = 1
                    ...
                    and t.created_at < date''''#v_load_date_txt#'''' + interval ''''1 day''''
                    and coalesce(t.vizql_session, ''''?'''') not in (select distinct session from cross_utc_midnight_sessions where parent_process_name = ''''vizqlserver''''))
