...
declare
    ...
    v_sql_cur text;
    v_min_ts_date_s_serverlogs text;
begin							
		
		execute 'set local search_path = ' || p_schema_name;
                
        v_sql_cur := 'select to_char(min(ts), ''yyyy-mm-dd'') from s_serverlogs';
        execute v_sql_cur into v_min_ts_date_s_serverlogs;
        v_min_ts_date_s_serverlogs := 'date''' || v_min_ts_date_s_serverlogs || '''';
		
        if v_min_ts_date_s_serverlogs is null then
            return 0;
        end if;
		...
		v_sql := 'insert into p_serverlogs(
				...
				from
					(SELECT       				  
						...
						FROM 
							p_http_requests r					
						WHERE
                          created_at >= #v_min_ts_date_s_serverlogs# - 1 AND
				...
				from
					...
					select 
						   ...
						   , s.elapsed_ms
						   , s.start_ts
						   , min(s.ts) over (partition by s.host_name, s.parent_vizql_session) as session_start_ts_utc
        ...
        v_sql := replace(v_sql, '#v_min_ts_date_s_serverlogs#', v_min_ts_date_s_serverlogs);
