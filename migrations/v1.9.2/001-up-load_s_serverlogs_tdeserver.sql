			...
			with t_s_spawner as
				...
				from 
					(select 
							...
							null as k,
							pl.line as v,
							case when sp.filename_prefix = ''vizqlserver'' then sm.sessid end as parent_vizql_session,
							sp.parent_vizql_destroy_sess_ts as parent_vizql_destroy_sess_ts,
							case when sp.filename_prefix = ''dataserver'' then sm.sessid end as parent_dataserver_session,
							sm.ts_start as spawned_by_parent_ts,							
							sp.filename_prefix as parent_process_type,
							case when sp.filename_prefix = ''vizqlserver'' then sp.parent_vizql_site end as parent_vizql_site,
							case when sp.filename_prefix = ''vizqlserver'' then sp.parent_vizql_username end as parent_vizql_username,
							case when sp.filename_prefix = ''dataserver'' then sp.parent_vizql_site end as parent_dataserver_site,
							case when sp.filename_prefix = ''dataserver'' then sp.parent_vizql_username end as parent_dataserver_username,
							pl.elapsed_ms,
							pl.start_ts					
					from (select pl0.*, 
                          ...
                          from plainlogs pl0
                     	  left join
								...
								where 
									line like ''pid=%'' and 
									ts < now()::date + 2
								) pids on substring(pl0.filename from ''tdeserver_[a-z]+server_[0-9]+'')
