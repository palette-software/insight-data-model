...
begin							
		...
		v_sql := 		
				'CREATE or replace function #function_schema_name#.load_p_http_requests(p_schema_name text,  p_load_date date) returns bigint
				AS \$\$
				declare
					v_sql text;
				...
				begin
                            ...
                            perform check_if_load_date_already_in_table(p_schema_name, ''p_http_requests'', p_load_date, true);
				...
				from 
                    ...
                    left outer join (select
                                    ...
                                    where 1 = 1
                                        ...
                                        and created_at <= date''''#v_load_date_txt#'''' + interval ''''2 hours''''
                                    ) phr on (t.p_id = phr.http_requests_p_id)
                where 1 = 1
                    and phr.http_requests_p_id is null
                    and 
                    (
                    (t.created_at >= date''''#v_load_date_txt#'''' and t.created_at < date''''#v_load_date_txt#'''' + interval ''''1 day'''')
                    
                    or
                    
                    (t.created_at >= date''''#v_load_date_txt#''''  + interval ''''24 hours'''' and t.created_at <= date''''#v_load_date_txt#'''' + interval ''''26 hours''''
