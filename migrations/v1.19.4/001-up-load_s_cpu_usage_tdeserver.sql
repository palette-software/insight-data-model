...
begin        
    ...
    loop
        ...
        v_sql :=                 
                ...
                where
                    1 = 1
                    and slogs.process_name like ''tdeserver%''
                    and slogs.host_name = ''#host_name#''
                    and slogs.ts >= #v_load_date_txt#
                    and slogs.ts <= date''#v_load_date_txt#'' + interval''26 hours''
                ),
                t_slogs_agg as
                (
                 select host_name,
                ...
                FROM 
                    (select
                           ...
                           ,sites.id as slog_site_id                
                            ...
                            ,count(*) over (partition by tri.host_name, tri.process_id, tri.ts_rounded_15_secs) as session_count
                    from    
                        (select 
                            p_id
                           ,threadinfo_id
