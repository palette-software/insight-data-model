...
BEGIN	
        ...
        select
            ...
            coalesce(e.datasource_id, dc.datasource_id) as datasource_id,
            min(case when datasrv_sess.process_name = ''dataserver'' then coalesce(ds.name, dc_ds.name) end) over (partition by datasrv_sess.host_name, datasrv_sess.dataserver_session) AS datasource_name
        from        
            t_base b
            inner join   
                    (SELECT  
