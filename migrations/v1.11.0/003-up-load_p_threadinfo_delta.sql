...
BEGIN	
			...
			v_sql_cur := 'select to_char((select get_max_ts_date(''#schema_name#'', ''p_threadinfo_delta'')), ''yyyy-mm-dd'')';
			v_sql_cur := replace(v_sql_cur, '#schema_name#', p_schema_name);			
			...
			v_sql_cur := 'select to_char(min(ts), ''yyyy-mm-dd hh24:mi:ss.ms'') from threadinfo where ts > timestamp''' || to_char(v_max_ts_p_threadinfo_delta, 'yyyy-mm-dd hh24:mi:ss.ms') || ''' + interval ''30 seconds''';
			      ...
			      FROM (select 
                        ...
                        where 								
								p_id > coalesce((select
												...
												where
													ts_rounded_15_secs >= #max_ts_date_p_threadinfo_delta#
												), 0)
								and ts >= #max_ts_date_p_threadinfo_delta# - interval''2 hours'' and 
						...
						from
							...
							where									
								last_ti.ts_rounded_15_secs >= #max_ts_date_p_threadinfo_delta# - interval''12 hours''
