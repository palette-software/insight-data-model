...
BEGIN	
		...
		SELECT  
				...
				CASE WHEN MIN(http_user_agent) IS NULL THEN NULL /* if we don''t have a value on http_user_agent, then there isn''t the given vizql session in the p_http_requests table so it should be null. */
				...
				CASE WHEN MIN(http_user_agent) IS NULL THEN NULL 
				...
				CASE WHEN MIN(http_user_agent) IS NULL THEN NULL 
					...
					AS bootstrap_count,
				CASE WHEN MIN(show_count) = 0 THEN NULL 
					ELSE MIN(show_elapsed_secs) END AS show_elapsed_secs,
				CASE WHEN MIN(bootstrap_count) = 0 THEN NULL 
					ELSE MIN(bootstrap_elapsed_secs) END AS bootstrap_elapsed_secs,
				CASE WHEN MIN(bootstrap_count) = 0 or MIN(show_count) = 0 THEN NULL
