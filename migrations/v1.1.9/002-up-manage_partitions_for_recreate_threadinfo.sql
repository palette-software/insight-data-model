CREATE or replace function manage_partitions_for_recreate_threadinfo(p_schema_name text, p_base_table_name text, p_target_table_name text) returns int
AS $$
declare
	v_sql_cur text;
	c refcursor;
...
BEGIN
		...
		if p_base_table_name in ('threadinfo_old') then
			v_sql_cur := 'select distinct host_name::text as host_name from #schema_name#.threadinfo_old order by 1';
		end if;
		...
		if p_base_table_name in ('threadinfo_old') then
			v_sql_cur := 'select distinct ts::date d from #schema_name#.threadinfo_old
							 order by 1';
		end if;
