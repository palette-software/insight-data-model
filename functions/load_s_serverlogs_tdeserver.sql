  ...
  insert
    ...
    (
      select
        ...
        rtrim(substring(filename from ''^[a-z_]+''), ''_'') as filename_prefix,
      ...
      from
        ...
        group by
          host_name, filename, sess
        ) slog

      group by host_name, rtrim(substring(filename from ''^[a-z_]+''), ''_''), sess
    ),
    ...
    from
      (select
        ...
        null as k,
        pl.line as v,
        case when pl.filename like ''tdeserver_vizqlserver%'' or pl.filename not like ''tdeserver#_%'' escape ''#'' and sp.filename_prefix = ''vizqlserver'' then sm.sessid end as parent_vizql_session,
        sp.parent_vizql_destroy_sess_ts as parent_vizql_destroy_sess_ts,
        case when pl.filename like ''tdeserver_dataserver%'' or pl.filename not like ''tdeserver#_%'' escape ''#'' and sp.filename_prefix = ''dataserver'' then sm.sessid end as parent_dataserver_session,
        sm.ts_start as spawned_by_parent_ts,
        case when pl.filename like ''tdeserver_vizqlserver%'' or pl.filename not like ''tdeserver#_%'' escape ''#'' and sp.filename_prefix = ''vizqlserver'' then ''vizqlserver''
          when pl.filename like ''tdeserver_dataserver%'' or pl.filename not like ''tdeserver#_%'' escape ''#'' and sp.filename_prefix = ''dataserver'' then  ''dataserver''
        end as parent_process_type,
        case when pl.filename like ''tdeserver_vizqlserver%'' or pl.filename not like ''tdeserver#_%'' escape ''#'' and sp.filename_prefix = ''vizqlserver'' then sp.parent_vizql_site end as parent_vizql_site,
        case when pl.filename like ''tdeserver_vizqlserver%'' or pl.filename not like ''tdeserver#_%'' escape ''#'' and sp.filename_prefix = ''vizqlserver'' then sp.parent_vizql_username end as parent_vizql_username,
        case when pl.filename like ''tdeserver_dataserver%'' or pl.filename not like ''tdeserver#_%'' escape ''#'' and sp.filename_prefix = ''dataserver'' then sp.parent_vizql_site end as parent_dataserver_site,
        case when pl.filename like ''tdeserver_dataserver%'' or pl.filename not like ''tdeserver#_%'' escape ''#'' and sp.filename_prefix = ''dataserver'' then sp.parent_vizql_username end as parent_dataserver_username,
        pl.elapsed_ms,
        pl.start_ts
        ...
        from
          (select
            pl0.*,
            coalesce(max(case when pl0.line like ''pid=%'' then substr(pl0.line, 5) end) over (partition by pl0.filename),  substr(pids.line, 5))::bigint as process_id,
