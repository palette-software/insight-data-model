...
BEGIN	
		...
		WITH rownofilt AS (
			...
			FROM
				(
					...
					FROM #schema_name#.p_http_requests
				) rowno	
		...
		WHERE cpu_usage_ts_rounded_15_secs >= #max_ts_date_p_interactor_session#
			...
			AND cpu_usage_parent_vizql_session || ''::'' || cpu_usage_process_name NOT IN (SELECT DISTINCT vizql_session || ''::'' || process_name FROM #schema_name#.p_interactor_session)
