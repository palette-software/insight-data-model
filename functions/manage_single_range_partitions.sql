...
BEGIN
		...
		if p_table_name in ('p_cpu_usage_agg_report') then --year
			v_sql_cur := 'select distinct timestamp_utc::date d from ' || 's' || ltrim(p_table_name, 'p');
		elseif p_table_name in ('p_interactor_session') then --year
			v_sql_cur := 'select distinct session_start_ts::date d from ' || 's' || ltrim(p_table_name, 'p');
		elseif p_table_name in ('p_process_class_agg_report') then --month
			v_sql_cur := 'select distinct ts_rounded_15_secs::date d from '|| 's' || ltrim(p_table_name, 'p');
		elseif p_table_name in ('p_cpu_usage_bootstrap_rpt') then --month
			v_sql_cur := 'select distinct cpu_usage_ts_rounded_15_secs::date d from '|| 's' || ltrim(p_table_name, 'p');
		elseif p_table_name in ('p_serverlogs_bootstrap_rpt') then --day
			v_sql_cur := 'select distinct start_ts::date d from '|| 's' || ltrim(p_table_name, 'p');
		end if;
		...
		open c for execute (v_sql_cur);
			loop
				...
				if p_table_name in ('p_cpu_usage_agg_report', 'p_interactor_session') then --year
