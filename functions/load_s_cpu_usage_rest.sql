CREATE or replace function load_s_cpu_usage_rest(p_schema_name text, p_load_date date) returns bigint
AS $$
declare
	v_sql text;
	...
	loop
		...
		v_sql := 
				...
				SELECT
				  ...
				  tri.process_name,
				  coalesce(pc.process_class, ''Non-Tableau'') as process_owner,
				...
				FROM 
					...
					where
						...
						and process_name not in (''vizqlserver'', ''dataserver'', ''tabprotosrv'', ''tdeserver'', ''tdeserver64'')
				) tri
				 left outer join p_process_classification pc on (pc.process_name = tri.process_name
																 and pc.process_class = ''Tableau'')
