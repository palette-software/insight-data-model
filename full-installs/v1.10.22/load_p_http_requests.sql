...
FROM  
	t_requests r
left outer join #schema_name#.h_users u on (
	u.id  = r.user_id and
	u.site_id = r.site_id and
	r.created_at between u.p_valid_from and u.p_valid_to)
left outer join #schema_name#.h_system_users su on (
	su.id = u.system_user_id and
	r.created_at between su.p_valid_from and su.p_valid_to)															  															  
left outer join #schema_name#.h_workbooks wb on (
	wb.site_id = r.site_id and
	wb.repository_url = r.workbook_url and 
	r.created_at between wb.p_valid_from and wb.p_valid_to)
left outer join #schema_name#.h_projects p on (
	p.site_id = r.site_id and
	p.id = wb.project_id and
	r.created_at between p.p_valid_from and p.p_valid_to)
left outer join #schema_name#.h_users wb_u on (
	wb_u.id  = wb.owner_id and
	wb_u.site_id = wb.site_id and
	r.created_at between wb_u.p_valid_from and wb_u.p_valid_to)
left outer join #schema_name#.h_system_users wb_su on (
	wb_su.id = wb_u.system_user_id and
	r.created_at between wb_su.p_valid_from and wb_su.p_valid_to)
left outer join #schema_name#.h_sites s on (
	s.id = r.site_id and
	r.created_at between s.p_valid_from and s.p_valid_to)
left outer join #schema_name#.h_views v on (
	v.site_id = r.site_id and
